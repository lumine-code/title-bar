...
class MenuUpdater {
  static getTemplate() {
    const template = deepClone(lumine.menu.template);
    ...
    // Match the label with its mnemonic marker stripped. The marker only
    // appears in the win32 and linux menu files -- darwin's say "Packages"
    // plainly -- so matching the raw label sorted the submenu on two platforms
    // out of three.
    template.some((o) => {
      if (typeof o.label === "string" && Utils.parseAltKey(o.label).name === "Packages") {
        o.submenu?.sort((a, b) => {
