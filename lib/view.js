...
// src/atom-window.js picks for the window and dock icon, so every surface
...
// without faking atom.window.isDevMode()/atom.window.isSafeMode() at the global level.
...
class TitleBarView {
  ...
  initWindowControls() {
    this.windowSubscriptions = [
      atom.window.onDidMaximize(() => {
        this.windowState.maximized = true;
        this.updateMaximizeControl();
      }),
      atom.window.onDidUnmaximize(() => {
        this.windowState.maximized = false;
        this.updateMaximizeControl();
      }),
      atom.window.onDidEnterFullScreen(() => {
        this.windowState.fullscreen = true;
        this.updateFullscreenState();
      }),
      atom.window.onDidLeaveFullScreen(() => {
        this.windowState.fullscreen = false;
        this.updateFullscreenState();
      }),
      atom.window.onDidBlur(() => {
        document.body.click();
      }),
      atom.window.onDidFocus(() => {
        void this.syncWindowState();
      }),
    ];
  ...
  restoreCapturedBounds() {
    ...
    return Promise.all([atom.window.setPosition(x, y), atom.window.setSize(width, height)]);
  }
  ...
  async toggleMaximized() {
    ...
    const isMaximized = await atom.window.isMaximized();
    ...
    if (shouldMaximize) {
      await atom.window.maximize();
    } else {
      await atom.window.unmaximize();
      if (!(await atom.window.isMaximized())) {
  ...
  async handleTitleBarDoubleClick(event) {
    ...
    if (process.platform === "darwin") {
      const action = await atom.app.getUserDefault("AppleActionOnDoubleClick", "string");

      if (action === "Minimize") {
        await atom.window.minimize();
        return;
      }
  ...
  updateLaunchMode() {
    const launchMode = resolveLaunchMode({
      ...
      sourceMode: Boolean(process.defaultApp) && !atom.window.isSpecMode(),
    });
    ...
    this.logoImage.src = pathToFileURL(
      path.join(atom.app.getResourcePath(), "resources", "app-icons", iconFile),
    ).href;
  }
