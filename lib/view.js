...
// How much room the menu has before it would run into the centred window
// title, measured from whichever edge the theme anchors the menu to.
...
class TitleBarView {
  ...
  // The menu manager has no "the template changed" event, so stand in front of
  // the method that rebuilds it. Nothing else re-reads `lumine.menu.template`,
  // and a package adding a menu item is exactly what has to reach this bar.
  attachMenuUpdater() {
  ...
  detachMenuUpdater() {
    ...
    // `delete` rather than assignment, or the prototype method ends up
    // shadowed by an own property of its own body for the rest of the session.
    if (this.hadOwnMenuUpdate) {
      lumine.menu.update = this.originalMenuUpdateFn;
    } else {
      delete lumine.menu.update;
    }
    this.originalMenuUpdateFn = undefined;
  }
  ...
  async handleTitleBarDoubleClick(event) {
    ...
    if (process.platform === "darwin") {
      const action = await lumine.application.getUserDefault("AppleActionOnDoubleClick", "string");
