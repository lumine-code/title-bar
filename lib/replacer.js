...
class TitleBar {
  ...
  activate() {
    // The bar belongs on a frameless window only. With `core.titleBar` set to
    // "native" the operating system draws one, and a second one below it is
    // not a configuration anyone asked for. The setting needs a relaunch to
    // take effect, so reading it once here is enough.
    if (lumine.config.get("core.titleBar") === "native") {
      return;
    }
