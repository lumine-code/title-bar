...
// Long enough for the menu's own focus restoration to settle before the
// command runs, so it sees the editor rather than the closing menu.
const CLOSE_SETTLE_DELAY = 10;
...
class MenuItem {
  ...
  static createMenuItem(menuItem) {
    ...
    // A reopen-project label is a filesystem path rather than authored text: it
    // names no accelerator, and an `&` in it belongs to the directory's name.
    const mnemonics = menuItem.command !== "application:reopen-project";
    const altKey = mnemonics ? Utils.parseAltKey(menuItem.label).key : null;
    if (altKey !== null) {
      itemEl.setAttribute("alt-trigger", altKey);
    }
    self.altTrigger = altKey ?? undefined;

    const menuItemName = document.createElement("span");
    menuItemName.classList.add("menu-item-name");
    if (menuItem.label === "VERSION") {
      menuItemName.textContent = `Version ${lumine.application.getVersion()}`;
    } else {
      Utils.renderAltKey(menuItemName, menuItem.label, { mnemonics });
    }
    ...
    if (
      ...
      itemEl.addEventListener("contextmenu", (e) => {
        e.stopPropagation();
        e.preventDefault();
        lumine.history.removeProject(paths);
      });
      if (paths.length > 1) {
        menuItemName.replaceChildren();
  ...
  onChildMouseEnter(target) {
    SubmenuParent.onChildMouseEnter(this, target);
  }

  onChildMouseMove(target) {
    SubmenuParent.onChildMouseMove(this, target);
  }
  ...
  // Where a command lands: the element that was right-clicked for a context
  // menu, the active editor or pane for the application menu.
  ...
  // Runs the item's command, on three routes -- because not everything a menu
  // item names is a command. A reopen-project entry opens a window, and the
  // handful in `exceptionCommands` belong to the main process rather than to
  // this renderer. Always a promise, so a caller can sequence on it whichever
  // route was taken.
  execCommand(target = undefined) {
    if (this.command === undefined) {
      return Promise.resolve();
    }

    if (MenuItem.isReopenProjectCommand(this.command, this.commandDetail)) {
      return Promise.resolve(MenuItem.reopenProjectInNewWindow(this.commandDetail));
    }
    ...
    return Promise.resolve(
      lumine.commands.dispatch(
        target ?? this.resolveDispatchTarget(),
        this.command,
        this.commandDetail,
      ),
    );
  ...
  // Closes the menu first, so the command runs against the editor rather than
  // against a menu that still holds focus. The target is resolved before the
  // close, because closing changes what is active.
  ...
  ensureKeystroke() {
    ...
    // A context-menu item resolves at the element that was right-clicked,
    // because that is where its command dispatches. A menu-bar item has no
    // such element and passes `undefined`, which leaves `findKeyBindings`
    // unfiltered exactly as before.
    const keyStrokes = lumine.keymaps.findKeyBindings({
      command: this.command,
      target: this.targetElement,
    });
    if (keyStrokes.length > 0) {
      // Given a target, `findKeyBindings` returns only the bindings that
      // apply, ordered innermost element first and strongest binding first, so
      // the winner is the first. Without one the array is in registration
      // order and the last is the closest thing to a winner it can offer.
      const binding = this.targetElement ? keyStrokes[0] : keyStrokes[keyStrokes.length - 1];
      this.keystrokeElement.textContent = Utils.formatKeystroke(binding.keystrokes);
    }
  }
